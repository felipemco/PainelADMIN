...
$(document).ready(function() {
    ...
    // Adicione um ouvinte de evento para capturar as alterações e salvar
    $('#styleSelector').on('change', 'input, select', function() {
        saveSettings();
    });
});
...
function loadSavedSettings() {
    // Obtenha as configurações salvas do Armazenamento Local
    const savedSettings = JSON.parse(localStorage.getItem('layoutSettings'));

    // Aplique as configurações carregadas aos elementos correspondentes no DOM
    if (savedSettings) {
        // Exemplo: $('#sidebar-position').prop('checked', savedSettings.sidebarPosition);
        // Ajuste esta linha de acordo com os elementos específicos do seu layout
    }
}

// Função para salvar configurações
function saveSettings() {
    // Crie um objeto para armazenar as configurações
    const settings = {
        // Exemplo: sidebarPosition: $('#sidebar-position').prop('checked'),
        // Ajuste esta linha de acordo com os elementos específicos do seu layout
    };

    // Converta o objeto para uma string JSON e salve no Armazenamento Local
